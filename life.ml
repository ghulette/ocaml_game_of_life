type world = { width : int; height : int; cells : bool array array }
...
let flip_cell earth x y = earth.cells.(x).(y) <- not earth.cells.(x).(y)
...
let neighbors earth xi yi =
  ...
  in
  List.fold_left
    (fun n (x, y) -> n + if is_alive earth (xi + x) (yi + y) then 1 else 0)
    0 idxs

let rule x = function 0 | 1 -> false | 2 -> x | 3 -> true | 4 | _ -> false
